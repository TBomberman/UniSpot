...
/// A `UniSpotDataSource` identifies a specific contract (given by its Wormhole `emitter`) on
/// a specific blockchain (given by `chain_id`).
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, Hash, JsonSchema)]
pub struct UniSpotDataSource {
    pub emitter:  Binary,
    pub chain_id: u16,
}
...
pub struct ConfigInfo {
    ...
    pub data_sources:               HashSet<UniSpotDataSource>,
    ...
    // governance messages, whereas the one above is generated by UniSpot and only applicable to governance
    // source transfers.
